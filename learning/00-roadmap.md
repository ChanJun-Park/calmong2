...
2. **[진행중] 선택값 추출** — 뷰포트 중앙에 있는 아이템의 인덱스를 상태로 뽑아내기 (`ScalingLazyListState.centerItemIndex`에 해당하는 것을 `LazyListState.layoutInfo`로 직접 계산). ([`step-02-center-item-index.md`](./step-02-center-item-index.md))
3. **무한 순환 매핑** — `LARGE_NUMBER_OF_ITEMS` + `optionsOffset` + 모듈로 매핑 트릭으로 무한 스크롤 흉내내기 (`PickerState.kt:622-638` 분석 기반).
4. **상태 호이스팅 + 복원** — `PickerState : ScrollableState` + `rememberSaveable`/`Saver`로 재사용 가능한 상태 클래스 만들기, `scrollToOption`/`animateScrollToOption` 구현.
