...
6. **[완료]** **플링/스냅 커브 튜닝** — `CustomSnapFlingBehavior`로 Wear의 decay+cubic-bezier 이어붙이기 기법을 직접 포팅. 추가로 "release 시점엔 안 바뀌었지만 decay가 끝난 뒤에도 안 바뀐 상태라면 이동 방향으로 반드시 한 칸 이동" 기능을 자체 설계해서 얹음(`DragInteraction.Start`로 드래그 시작 시점의 중앙 인덱스 추적). 부호 버그(`centerItemScrollOffset`) 한 번 있었고 수정함. ([`step-06-fling-snap-curve.md`](./step-06-fling-snap-curve.md))
7. **[완료]** **접근성** — TalkBack 지원. 처음엔 "휠 전체를 하나의 조정 가능한 컨트롤"(`progressBarRangeInfo`+`setProgress`, Slider 패턴)로 시도했다가, **아이템 하나하나가 개별 포커스+선택 가능하게** 하는 방향으로 전환. `Modifier.selectable` + 아이템별 `clearAndSetSemantics{}`(빈 블록으로 Text 자체의 raw 시맨틱만 제거, 바깥 `selectable`/`contentDescription`은 유지)로 구현. 컨테이너의 `CollectionInfo`를 실제 옵션 개수로 오버라이드해서 "10만 개 항목" 오발화는 해결(정확한 개수 발화는 안 되지만 문제 없다고 판단). `Role`은 시각적 형태와 안 맞는다는 이유로 `RadioButton` 대신 `Button` 유지. ([`step-07-accessibility.md`](./step-07-accessibility.md))
